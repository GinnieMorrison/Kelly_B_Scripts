...
frequencyData<-frequencyData[with(frequencyData,order(Chromosome,Physical.Position)),]
   png("Kelly_Syn0Syn6_Chr1_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==1,"Physical.Position"]
   B_chr1<-splineAnalyze(frequencyData[frequencyData$Chromosome==1,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T) 
   dev.off()
   Chr1_windows<-B_chr1$windowData
   Chr1_windows$Chromosome<-rep(1,dim(Chr1_windows)[1])
 
   png("Kelly_Syn0Syn6_Chr2_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==2,"Physical.Position"]
   B_chr2<-splineAnalyze(frequencyData[frequencyData$Chromosome==2,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr2_windows<-B_chr2$windowData
   Chr2_windows$Chromosome<-rep(2,dim(Chr2_windows)[1])

   png("Kelly_Syn0Syn6_Chr3_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==3,"Physical.Position"]
   B_chr3<-splineAnalyze(frequencyData[frequencyData$Chromosome==3,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr3_windows<-B_chr3$windowData
   Chr3_windows$Chromosome<-rep(3,dim(Chr3_windows)[1])
   
   png("Kelly_Syn0Syn6_Chr4_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==4,"Physical.Position"]
   B_chr4<-splineAnalyze(frequencyData[frequencyData$Chromosome==4,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr4_windows<-B_chr4$windowData
   Chr4_windows$Chromosome<-rep(4,dim(Chr4_windows)[1])

   png("Kelly_Syn0Syn6_Chr5_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==5,"Physical.Position"]
   B_chr5<-splineAnalyze(frequencyData[frequencyData$Chromosome==5,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr5_windows<-B_chr5$windowData
   Chr5_windows$Chromosome<-rep(5,dim(Chr5_windows)[1])

   png("Kelly_Syn0Syn6_Chr6_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==6,"Physical.Position"]
   B_chr6<-splineAnalyze(frequencyData[frequencyData$Chromosome==6,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr6_windows<-B_chr6$windowData
   Chr6_windows$Chromosome<-rep(6,dim(Chr6_windows)[1])

   png("Kelly_Syn0Syn6_Chr7_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==7,"Physical.Position"]
   B_chr7<-splineAnalyze(frequencyData[frequencyData$Chromosome==7,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr7_windows<-B_chr7$windowData
   Chr7_windows$Chromosome<-rep(7,dim(Chr7_windows)[1])

   png("Kelly_Syn0Syn6_Chr8_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==8,"Physical.Position"]
   B_chr8<-splineAnalyze(frequencyData[frequencyData$Chromosome==8,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr8_windows<-B_chr8$windowData
   Chr8_windows$Chromosome<-rep(8,dim(Chr8_windows)[1])

   png("Kelly_Syn0Syn6_Chr9_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==9,"Physical.Position"]
   B_chr9<-splineAnalyze(frequencyData[frequencyData$Chromosome==9,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T)
   dev.off()
   Chr9_windows<-B_chr9$windowData
   Chr9_windows$Chromosome<-rep(9,dim(Chr9_windows)[1])

   png("Kelly_Syn0Syn6_Chr10_method4.png")
   pos<-frequencyData[frequencyData$Chromosome==10,"Physical.Position"]
   B_chr10<-splineAnalyze(frequencyData[frequencyData$Chromosome==10,"KellyTest2"]^2,map=pos,method=4,plotRaw=T,plotWindows=T) 
   dev.off()
...
write.table(Windowed_Chrs,"Syn0Syn6_Windows_method4.txt",row.names=F,sep='\t')
...
sd.Bwin = sqrt(2*delta_m4)*((quantile(Windowed_Chrs$Window.B,na.rm=T)[4] -quantile(Windowed_Chrs$Window.B,na.rm=T)[2]) / (qchisq(0.75,df=delta_m4) - qchisq(0.25,df=delta_m4)))

Windowed_Chrs$Bstar <- delta_m4+sqrt(2*delta_m4)*( (Windowed_Chrs$Window.B - Windowed_Chrs$SNPcount) / sd.Bwin  )

Windowed_Chrs$Window.p <- pchisq(Windowed_Chrs$Bstar,df=delta_m4,lower.tail=F) # P.values  

Windowed_Chrs$Window.fdr <- p.adjust(Windowed_Chrs$Window.p,method="fdr") #Kelly says to use this correction of the B score pvalue

write.table(Windowed_Chrs, "Kelly_GenWin_method4_Syn0Syn6.txt",row.name=F,sep='\t')
q()
n
